"""Execution harness for docs/PREREG_K_GRID_FREQ_v0.4.md.

Replication of the v0.3 statistic (zero-lag Pearson correlation of
linearly detrended windows) on a SECOND, non-overlapping continuous
QI_PT=0/QI_TR=0 window from the same file -- everything except the
window boundaries is copied unchanged from _grid_freq_k_run_v0_3.py.
"""
...
# Second-longest continuous QI_PT=0 AND QI_TR=0 block, excluding the
# block used in v0.1/v0.2/v0.3 (start_row=2071846, len=238902), frozen
# in PREREG_K_GRID_FREQ_v0.4.md BEFORE computing the statistic.
...
def main() -> None:
    ...
    result = {
        "prereg": "docs/PREREG_K_GRID_FREQ_v0.4.md",
        ...
        "replicates": "docs/RESULT_K_GRID_FREQ_v0.3.md (independent, non-overlapping window)",
    }

    if insufficient_data:
        result["verdict"] = "INCONCLUSIVE"
        result["reason"] = f"only {n} windows, below MIN_WINDOWS_FOR_TEST={MIN_WINDOWS_FOR_TEST}"
        Path("docs/RESULT_K_GRID_FREQ_v0.4.json").write_text(json.dumps(result, indent=2, default=str) + "\n")
